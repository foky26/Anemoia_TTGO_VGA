...
IRAM_ATTR void Apu2A03::cpuWrite(uint16_t addr, uint8_t data)
{
    ...
    case 0x4015:
        ...
        else { pulse1_enable = false; pulse1.len_counter.timer = 0; }

        if ((data >> 1) & 0x01) { pulse2_enable = true; }
        else { pulse2_enable = false; pulse2.len_counter.timer = 0; }

        if ((data >> 2) & 0x01) { triangle_enable = true; }
        else { triangle_enable = false; triangle.len_counter.timer = 0; }

        if ((data >> 3) & 0x01) { noise_enable = true; }
        else { noise_enable = false; noise.len_counter.timer = 0; }
...
inline void Apu2A03::generateSample()
{
    ...
    sample = (uint16_t)((sample * volume) >> 7);	
    sample += prev_sample;
    sample >>= 1;
    sample &= 0xFF;
    prev_sample = sample;
    ...
    audio_buffer[index + 1] = sample;  // Stereo
    ...
    if (buffer_index >= AUDIO_BUFFER_SIZE)
    {
        buffer_index = 0;
        static size_t dummy;
        i2s_write(I2S_NUM_0, audio_buffer, sizeof(audio_buffer), &dummy, pdMS_TO_TICKS(10));
    }
}
